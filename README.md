...
Бот сохраняет публичные идентификаторы (UID, email или адрес кошелька) без API‑ключей.
Если нужно подключение API‑ключей, потребуется отдельная реализация безопасного хранения.
